...
/***/ function(module, exports, __webpack_require__) {
	...
	var imagesloaded = isBrowser ? __webpack_require__(268) : null;
	...
	function MasonryComponent(React) {
		...
		bounceupdate: _.throttle(function(x){
			...
			if (that.state.loaded[x.props.src] === 1) {

			}
			else if (that.state.loaded[x.props.src] === 2) {
			    break;
			}
			else {
			    console.log('will wait for', x.props.src, i);
			    
			    (function(x) {
				return function() {
				    window.loadtimeout = setTimeout(function(){
					console.log('not waiting', x.props.src);
					that.state.loaded[x.props.src] = 2;
			...
			if(that.state.loaded[x.props.src] === 2) {
			    return x;
			}
		    });
