...
/***/ function(module, exports, __webpack_require__) {
	...
	var Pic = _react2.default.createClass({
	    ...
	    componentWillMount: function componentWillMount() {
	        ...
	        $('<img />').css({ position: 'absolute', left: '-10000px' }).load(function () {
	            that.setState({ opacity: 1, src: that.props.src });
	        }).attr({ src: this.props.src });
	    },
	    nav: function nav(k, v) {},
	    render: function render() {
	        var src = this.state.src;
	        return _react2.default.createElement('img', { src: src, style: { opacity: this.state.opacity, transition: 'opacity 1s ease' }, ref: 'img' });
	    }
	});
