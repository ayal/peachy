...
/***/ function(module, exports, __webpack_require__) {
	...
	var listpick = function listpick() {
	    ...
	    var arr = [];
	    for (var i = 0; i < 8; i++) {
	        arr.push(list[(x.getMinutes() * x.getHours() + i + 10) % list.length]);
	    }
	    return arr;
	};
	...
	var Pic = _react2.default.createClass({
	    ...
	    componentWillMount: function componentWillMount() {
	        ...
	        console.log('will load pic', this.props.src);
	        $('<img />').css({ position: 'absolute', left: '-10000px' }).load(function () {
	            var img = this;
	            that.setState({ opacity: 0 });
	            setTimeout(function () {
	                console.log('loaded pic', that.props.src);
	                that.setState({ opacity: 1, src: that.props.src, width: $(img).width(), height: $(img).height() });
	            }, 1000);
	        }).attr({ src: this.props.src }).appendTo('body');
	    },
	    nav: function nav(k, v) {},
	    render: function render() {
	        console.log('rendering pic', this.state.src);
