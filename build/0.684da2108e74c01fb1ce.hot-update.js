...
/***/ function(module, exports, __webpack_require__) {
	...
	window.softclean = function (e, t) {
	    return e.replace('"', '').split('ft')[0];
	};

	window.clean = function (e, t) {
	    return t ? e ? e.toLowerCase().replace('"', '').split('ft')[0].replace(/^the\s|\sthe\s|\sand\s| ep$/gim, " ").replace(/part/gim, "pt").replace(RegExp("[^\\p{L}a-zA-Z0-9]", "gim"), "").replace("around", "round").trim(" ") : "" : e ? e.toLowerCase().replace(/^the\s|\sthe\s|\sand\s| ep$/gim, " ").replace(/\(.*?\)/gim, "").replace(/part/gim, "pt").replace(RegExp("[^\\p{L}a-zA-Z0-9]", "gim"), "").replace("around", "round").trim(" ") : "";
	};
	...
	var App = _react2.default.createClass({
	    ...
	    componentWillMount: function componentWillMount() {
	        var that = this;
	        var toset = {};
	        var chosen = ['http://pitchfork.com/rss/reviews/best/tracks/', list[0], 'http://www.huhmagazine.co.uk/blog/rss/feed.php', 'http://feeds2.feedburner.com/itsnicethat/SlXC'].concat(_toConsumableArray(list.slice(1, 8)));
	...
	var Pic = _react2.default.createClass({
	    ...
	    render: function render() {
	        ...
	        return _react2.default.createElement('img', { src: this.props.src, style: { opacity: 0 }, onLoad: this.onload, ref: 'img' });
	    }
	});
